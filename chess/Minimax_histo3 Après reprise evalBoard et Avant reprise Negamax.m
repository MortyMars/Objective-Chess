...
Depuis la première refonte de l'interface intégrant le réglage de la valeur de NUMBER_MOVE_AHEAD par le code, la
constante a été transformée en variable globale et a été transférée dans le fichier Util.h  */
...
@implementation Minimax
   ...
   +(int)NegamaxForSide:(Side)side
      ...
      else
      {
         ...
         NSLog(@"\n\t\tNegaFS : Meilleure évaluation retournée par Negamax = %d",best);
                    ...
                    board:(ChessBoard *)board
   {
      ...
      if (side == sideBlack) evalBoard = -evalBoard;
      return evalBoard;
