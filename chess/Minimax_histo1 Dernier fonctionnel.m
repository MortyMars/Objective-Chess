...
@implementation Minimax
                      ...
                      board:(ChessBoard *)board
   {
      ...
      else
      {
         ...
         /* // Bloc cosmétique, potentiellement désactivable
         int eval = [self EvalBoardForSide:side board:board];
         NSLog(@"\n\n\tLe 'bestMove' IA (%@) est %@ \tpour un 'bestScore' cible de %d \n\tL' 'eval' courante du board est de %d \n\n",(side==sideWhite)?@"Blancs":@"Noirs",[MCNmoveToStr Modif00EnA1:bestMove surBoard:board],bestScore,eval);
         // Fin de bloc cosmétique  */
   ...
   // La méthode est appelée initialement par bestMoveFS avec, depth fixée à 3 (*), alpha à -INT_MAX, et bêta à +INT_MAX
      ...
      else
      {
         ...
         for (Move *moveEnCours in [self PossibleMovesForSide:otherSide board:board])
         {
            ...
            if (score > best) {
               ...
               NSLog(@" \n On est en depth = %d (NUMBER_MOVE_AHEAD = %d) pour un coup %@",
                     depth, NUMBER_MOVES_AHEAD, (otherSide == 1)? @"Noirs":@"Blancs");
               NSLog(@" Ancienne évaluation abandonnée = %d \n Nouvelle évalution  retenue    = %d",best, score);
               if (best > alpha) {
                    ...
                    board:(ChessBoard *)board
   {
      ...
      On balaye chaque case de l'échiquier considéré, pour obtenir une valeur globale totalisant la valeur
      de chacune des pièces encore présentes. À noter que la valeur relative des pièces (au Roi près) est celle
      communément attribuée par la sphère échiquéenne */
         ...
         for (int y = 0; y < 8; y++)   // balayage des ordonnées
         {
            ...
            if (piece)
            {
               ...
               switch (piece.type)
               {
                  ...
                  essentiel de lui accorder une grande valeur afin d'inciter l'IA à choisir, par appat du gain,
               ...
               evalBoardRetournee += value * ((piece.side == side)? 1 : -1);      // INCRÉMENT VALEUR RETOURNÉE
               
               /* VALEUR POUR L'AFFICHAGE : Si la pièce est Blanche, alors sa valeur s'ajoute au total, sinon elle
               est déduite (par convention une éval positive indique un avantage aux Blancs et une éval négative
               un avantage aux Noirs) */
               ...
               global selon la couleur en cours, ce qui est cohérent avec la nécessité d'inverser cette valeur,
               imposée par l'algo Négamax quand c'est aux Noirs de jouer...
               
               TODO - VÉRIFIER QUE LE SIGNE PORTÉ PAR L'ÉVALUATION INFLUENCE CORRECTEMENT L'IA DANS
               SON CHOIX DU MEILLEUR COUP POUR ELLE, QUELLE QUE SOIT LA COULEUR QU'ELLE JOUE */
   
            }
         }
      }
      
      /* PRISE EN COMPTE DE LA MISE EN ÉCHEC OU EN ÉCHEC ET MAT */
      Side otherSide = (side == sideWhite)? sideBlack:sideWhite;
      
      if ([self TestEchecRoiSide:otherSide inBoard:board]) {
         if ([self PossibleMovesForSide:otherSide board:board].count == 0) { // otherSide est Mat
            evalBoardRetournee += +100000;
            evalBoardAffichee  += +100000 * ((side == sideWhite)? 1 : -1);
         }
         else {                                                              // otherSide est Echec
            evalBoardRetournee +=   +5000;
            evalBoardAffichee  +=   +5000 * ((side == sideWhite)? 1 : -1);
         }
      }
      else if ([self TestEchecRoiSide:side inBoard:board]) {
         if ([self PossibleMovesForSide:side board:board].count == 0) {      // side est Mat
            evalBoardRetournee += -100000;
            evalBoardAffichee  += -100000 * ((side == sideWhite)? 1 : -1);
         }
         else {                                                              // side est Echec
      ...
      /* FIN DE PRISE EN COMPTE DE LA MISE EN ÉCHEC OU EN ÉCHEC ET MAT  */
      
      
      /* PRISE EN COMPTE DES PIONS ARRIVÉS SUR LEUR AVANT-DERNIÈRE RANGÉE
      ON LEUR ACCORDE +900 COMME S'ILS ÉTAIENT DÉJÀ PROMUS EN DAMES
      ...
      if (sideJoueur == sideWhite) {
         for (int x = 0; x < 8; x ++) {
            ...
            if ((pionB.type == Pion) && (pionB.side == sideWhite)){
               evalBoardRetournee += 900 *((pionB.side == side)? 1 : -1);
               evalBoardAffichee  += 900 *((side == sideWhite) ? 1 : -1);
               //NSLog(@"\n1 Un pion Blanc est à 1 coup de sa promotion...\n");
            }
            Piece *pionN = board->pieceCase[x][1];  // Pion Noir en rangée 1
            if ((pionN.type == Pion) && (pionN.side == sideBlack)){
               evalBoardRetournee += 900 *((pionN.side == side)? 1 : -1);
               evalBoardAffichee  += 900 *((side == sideWhite) ? 1 : -1);
               //NSLog(@"\n2 Un pion Noir est à 1 coup de sa promotion...\n");
            }
         }
      }
      ...
      if (sideJoueur == sideBlack) {
         for (int x = 0; x < 8; x ++) {
            ...
            if ((pionB.type == Pion) && (pionB.side == sideWhite)){
               evalBoardRetournee += 900 *((pionB.side == side)? 1 : -1);
               evalBoardAffichee  += 900 *((side == sideWhite) ? 1 : -1);
               //NSLog(@"\n3 Un pion Blanc est à 1 coup de sa promotion...\n");
            }
            Piece *pionN = board->pieceCase[x][6];  // Pion Noir en rangée 6
            if ((pionN.type == Pion) && (pionN.side == sideBlack)){
               evalBoardRetournee += 900 *((pionN.side == side)? 1 : -1);
               evalBoardAffichee  += 900 *((side == sideWhite) ? 1 : -1);
               //NSLog(@"\n4 Un pion Noir est à 1 coup de sa promotion...\n");
            }
         }
      }
      ...
      // TEST
      // TODO VOIR COMPORTEMENT DU JEU
      // INVERSION EVALBOARD QD C'EST L'ADVERSAIRE DE L'IA QUI JOUE
      // if (side == sideWhite) evalBoardRetournee = -evalBoardRetournee;
      // FIN TEST
   ...
   //*************************************************************************************************************
   // MCN - Méthode de Classe (inspirée de 'PossibleMoveForSide')
   // Détection des positions d'Échec en faveur du coté 'Side' (autrement dit Roi 'otherSide' en échec)
   // La méthode vise à déterminer tous les 'moves' possibles coté 'side' d'un 'board' et à examiner si à chaque
   // destination de chacun de ces 'moves' on trouve le Roi adverse, auquel cas cela signifie qu'il est en échec.
   // De par sa lourdeur relative comparée à sa version lite 'TestEchecRoiSide', cette méthode est à réserver aux
   // besoins d'affichage de la chaine de description complète de l'échec car c'est elle qui recherche les positions
   // d'échec multiples et qui gère les messages d'alerte correspondant. NOTER que le Mat est détecté par un autre
   // test -très basique- permettant de déterminer si un protagoniste ne dispose plus d'aucun coup autorisé.
   // TODO : Voir s'il n'y a pas de dégraissage de code à faire
   ...
   // *************************************************************************************************************
   // MCN - Méthode de Classe
   // Version "lite" de 'TestEchecFavSide' (renvoyant quant à elle une chaine décrivant la situation vis-à-vis de
   // l'échec) visant pour sa part à répondre RAPIDEMENT à la question de savoir si le Roi est en échec, en se moquant
   // d'être capable de dire si l'échec est simple ou double et si le mat est confirmé...
            ...
            if (pieceAdv)
            {
               if (pieceAdv.side == otherSide)
               {
                  ...
                  for (Pos *possibleDest in PosAcceptees)
                  {
                     ...
                     // DÉTECTION MISE EN ÉCHEC  >>et sur chacune de ces cases destinations on regarde si on trouve
                     // notre Roi, auquel cas nous sommes en situation d'Échec :-(
